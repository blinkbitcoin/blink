...
const processTxForWallet = async ({
  ...
  for (const { sats, address, vout } of tx.rawTx.outs) {
    ...
    const result = await addSettledTransaction({
      ...
      satoshis,
      address,
      // These legacy wallet addresses are generated by LND and only stored on
      // the wallet document. The receive is onchain, but the physical asset
      // appears in LND's wallet, so the ledger asset debit must follow LND.
