...
describe("CsvWalletsExport", () => {
  ...
  it("exports usd wallet rows with a cents-denominated fee", async () => {
    const walletDescriptor = UsdWalletDescriptor(crypto.randomUUID() as WalletId)
    ...
    const cells = rows[0].split(",")
    expect(cells[headers.indexOf("currency")]).toEqual("USD")
    expect(cells[headers.indexOf("fee")]).toEqual("10")
    expect(cells[headers.indexOf("feeUsd")]).toEqual("0.1")
    expect(cells[headers.indexOf("usd")]).toEqual("1")
  })
})
